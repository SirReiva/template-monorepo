...
const findFile = async (basePath: string) => {
	...
	if (dirStat?.isDirectory())
		return await findByExtension(`${basePath}${sep}index`);
...
export const resolve: ResolveHook = async (specifier, context, next) => {
	...
	if (
		...
		fileURLToPath(parentURL).startsWith(nodeModulesFolder)
	) {
	...
	try {
		if (specifier.startsWith(workspaceName))
			return await next(await resolveAliasImport(specifier), context);

		const filefullName = fsResolve(
